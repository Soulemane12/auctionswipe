...
import { getAllBiddedAuctions, getWins, recordWin, WinRecord } from "@/lib/history";
...
interface WinToast { id: number; auction: string; amount: string; }
...
export default function Dashboard() {
  ...
  const [localWins, setLocalWins] = useState<WinRecord[]>([]);
  ...
  useEffect(() => {
    ...
    if (requestedTab === "bids" || requestedTab === "won" || requestedTab === "agent") {
      setTab(requestedTab);
    }
  }, [searchParams]);

  useEffect(() => {
    setLocalWins(getWins(address));
  }, [address]);
  ...
  useEffect(() => {
    ...
    socket.on("auction:ended", (e: { auctionAddress: string; winner?: string; amount?: string }) => {
      const auctionAddr = e.auctionAddress?.toLowerCase();
      const winner = e.winner?.toLowerCase();
      const userAddr = address.toLowerCase();

      if (!auctionAddr || !winner || winner !== userAddr) return;

      recordWin(e.auctionAddress, address, {
        amount: e.amount ?? "0",
        timestamp: Date.now(),
      });
      setLocalWins(getWins(address));
      ...
      const toast = { id: Date.now(), auction: e.auctionAddress, amount: e.amount ?? "0" };
      setWinToasts((p) => [toast, ...p].slice(0, 3));
      setTimeout(() => setWinToasts((p) => p.filter((t) => t.id !== toast.id)), 12000);
    });
    return () => {
  ...
  const localWonAddrs = localWins.map((r) => r.auction?.toLowerCase()).filter(Boolean);
  const duneWonAddrs  = duneWon.map((r) => r.auction?.toLowerCase()).filter(Boolean);
  const allAuctions = [...new Set([...duneAddrs, ...localAuctions])];
  const wonAuctions = [...new Set([...localWonAddrs, ...duneWonAddrs])];
  ...
  return (
    <div style={{ minHeight: "100vh", background: BG, color: "#fff", fontFamily: FONT }}>
      {winToasts.length > 0 && (
        <div style={{ position: "fixed", top: 72, left: 16, zIndex: 200, display: "flex", flexDirection: "column", gap: 8, width: 320 }}>
          {winToasts.map((t) => (
            <div key={t.id} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: 13, fontWeight: 700, color: "#86efac", margin: 0 }}>You won an auction!</p>
                <p style={{ fontSize: 11, color: "rgba(134,239,172,0.75)", margin: "2px 0 0", fontFamily: "monospace", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {t.auction.slice(0, 10)}… · {(Number(t.amount) / 1e18).toFixed(2)} tkn
                </p>
              </div>
              <Link href="/dashboard?tab=won" style={{ color: "#86efac", fontSize: 11, fontWeight: 700, textDecoration: "none", flexShrink: 0 }}>View →</Link>
              <button onClick={() => setWinToasts((p) => p.filter((x) => x.id !== t.id))}
                style={{ color: "rgba(134,239,172,0.45)", background: "none", border: "none", cursor: "pointer", fontSize: 18, padding: 0, lineHeight: 1 }}>×</button>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ maxWidth: 600, margin: "0 auto", borderLeft: BORDER, borderRight: BORDER, minHeight: "100vh" }}>
        ...
        <div style={{ padding: "20px 16px 0" }}>
          ...
          <div style={{ display: "flex", gap: 0, borderTop: BORDER, paddingTop: 14, paddingBottom: 16 }}>
            {[
              ...
              { n: wonAuctions.length, label: "Auctions won" },
        ...
        {tab === "won"   && <WonTab   duneWon={duneWon} localWins={localWins} wonAuctions={wonAuctions} duneLoading={duneLoading} />}
...
function WonTab({ duneWon, localWins, wonAuctions, duneLoading }: {
  duneWon:     DuneWonRow[];
  localWins:   WinRecord[];
  wonAuctions: string[];
  duneLoading: boolean;
}) {
  if (duneLoading && wonAuctions.length === 0) return <Skeleton count={2} />;
  ...
  return (
    <div>
      {wonAuctions.map((auctionAddress) => (
        <WonCard
          key={auctionAddress}
          auctionAddress={auctionAddress}
          duneRow={duneWon.find((row) => row.auction?.toLowerCase() === auctionAddress.toLowerCase())}
          localRow={localWins.find((row) => row.auction?.toLowerCase() === auctionAddress.toLowerCase())}
        />
      ))}
    </div>
  );
...
function WonCard({ auctionAddress, duneRow, localRow }: {
  auctionAddress: string;
  duneRow?: DuneWonRow;
  localRow?: WinRecord;
}) {
  ...
  const stateNum = state !== undefined ? Number(state) : -1;
  const payoutText =
    duneRow?.payout_tokens !== undefined
      ? `${duneRow.payout_tokens.toFixed(2)} tokens`
      : highestBid !== undefined
        ? `${(Number(highestBid) / 1e18).toFixed(2)} tkn`
        : localRow?.amount
          ? `${(Number(localRow.amount) / 1e18).toFixed(2)} tkn`
          : "—";

  const wonAt = duneRow?.block_time
    ? new Date(duneRow.block_time).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })
    : localRow?.timestamp
      ? new Date(localRow.timestamp).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })
      : null;
    ...
    <div style={{
      ...
      <div style={{
        ...
        {imageURI ? (
          ...
          <div style={{ width: "100%", height: "100%", background: "linear-gradient(135deg, #f4b40020, #e040fb20)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22 }} >
            🏆
          </div>
        )}
      </div>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <p style={{ color: "#536471", fontSize: 11, fontFamily: "monospace", margin: "0 0 4px" }}>
          {auctionAddress.slice(0, 10)}…{auctionAddress.slice(-4)}
        </p>
        <p style={{ fontSize: 14, fontWeight: 800, color: "#00e676", margin: "0 0 4px" }}>
          Winning bid: {payoutText}
        </p>
        {wonAt && (
